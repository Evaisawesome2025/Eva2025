...
// Presentational business website. Pure (no hooks, no fs) so it renders both
// server-side (when the account is persisted) and client-side (from localStorage
// on serverless hosts where the filesystem is ephemeral).
export default function BusinessSiteView({ account }) {
  ...
  return (
    <div className="bs">
      ...
      <div className="bar">
        ⚡ Now booking in {w.area} — call or text <a href={w.telHref}>{w.phone}</a>
      </div>
